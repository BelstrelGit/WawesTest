...
import service.{ClientsFromFile, TradeSystem}

class TestTrading extends FreeSpec {


  // val totalBalances = List(26470, 2140, 1600, 3480, 2210) // итоговая сумма балансов
  "Проверка стакана" - {
    // проверяем очередность ордеров с одной ценой - сначало более ранние ордера выполняются
    // пакетное исполнение используя цепочку ордеров
    // (если если например покупка не удволетворена с одного ордера, и есть доступные ордера с продажей)
    // ограничения на покупку (по болансу)
    // ограничения на продажу (по количеству наличия бумаг)
    "на покупку" in {
      TradeSystem.prepareSystemForTest
      val clients = new ClientsFromFile
